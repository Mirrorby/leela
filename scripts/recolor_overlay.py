...
LAYERS = [
    ...
    (
        ...
        # Светлый пастельный редизайн: раньше здесь был старый светлый
        # --gold-soft (0xF2,0xB4,0x4D) — специально светлый тон для
        # контраста на тёмной доске. На новой светлой пастельной доске
        # тот же светлый золотой почти не виден, поэтому теперь новый,
        # заметно более тёмный --gold-soft ("подъём/благословение" всё ещё
        # золотой, просто темнее для контраста на светлом фоне).
        (0xC6, 0x94, 0x1F),  # --gold-soft (светлая тема)
        (0x3A, 0x1E, 0x0C),
    ),
]
